...
class Agent:
    ...
    def judge(self, situation_id, player, response):
        ...
        verdict_text = self.llm.generate(
            verdict_prompt, max_tokens=20, temperature=0.0
        )
        ...
        # Phase 2: Story
        story_prompt = build_story_prompt(situation, player, verdict, response)
        story = self.llm.generate(story_prompt)

        # 🔒 HARD CONSISTENCY ENFORCEMENT
        if verdict == "DIE" and any(
            word in story.lower()
            for word in ["escape", "survive", "safe", "rescues"]
        ):
            # Regenerate with extreme constraint
            story_prompt += "\nREMINDER: The player dies. Describe death only."
            story = self.llm.generate(story_prompt)

        if verdict == "SURVIVE" and any(
        ...
        return {"player": player, "verdict": verdict, "story": story.strip()}
